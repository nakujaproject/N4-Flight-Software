 ...
 *
 * @file logger.h
 * @brief This file implements the onboard logging functions for the flight computer
 * We use the SerialFlash library by Paul, link to the library is provided in the references
 * section. If you are using more than one SPI flash memory, create an instance for each SPI flash memory
 * 
*/
...
class DataLogger {
    ...
    public:
        DataLogger(uint8_t cs_pin, uint8_t led_pin, char* filename, SerialFlashFile file, uint32_t filesize); // constructor
        bool loggerInit();
        void loggerFormat();
        void loggerInfo();
        bool loggerTest();
